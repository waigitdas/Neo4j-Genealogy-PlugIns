...
public class mss_dna_painter {
     ...
     public String create_csv(String rns) 
    {
        ...
        gen.neo4jlib.neo4j_qry.qry_to_csv(cq_all.replace("[[","[").replace("]]","]").replace("⦋","[").replace("⦌","]"),gen.neo4jlib.neo4j_info.project + "_dna_painter_" + gen.genlib.current_date_time.getDateTime() +  ".csv");
        
        //create Excel output for reference
        gen.excelLib.queries_to_excel.qry_to_excel(cq_all,gen.neo4jlib.neo4j_info.project +  "_DNA_Painter_Excel", "DNA_Painter-excel",  1, "", "0:##;1:###,###,###;2:###,###,###;3:####.#;4:#####", "", true,"UDF:\nreturn gen.mss.dna_painter_csv('" + rns +  "')\n\n cypher query:\n" +  cq_all + "\n\nLook in the import directory for the csv file DNA_Painter.csv which can be bulk loaded toDNA Painter.\n\nThe monophylytic ancestors are, by definition, on one side of the family tree. Since GFG does not know this a priori, the default in this outpiut is mmaternal.\nThis allows DNA Paint to leave all the segments on one set of chrromosomes (maternal).", false);
        return "completed";
    }
}
