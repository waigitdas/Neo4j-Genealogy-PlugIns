...
public class path_matches_for_indiv {
    @UserFunction
    @Description("Create list of DNA testee candidates for endogamy path triangulation.")
    ...
    public String triangulate_paths_candidates(
        ...
        get_aligned_paths(rn);
         return "";
            }

        
    public static void main(String args[]) {
        get_aligned_paths(4L);
    }
    
     public static String get_aligned_paths(Long rn) 
    {
        ...
        FileWriter fw = null;
        String fn ="path_triangulation_" + rn +  "_" + gen.genlib.current_date_time.getDateTime() + ".csv";
        ...
        //get in memory list of paths of proband from path knowledge graph
        String cq = "MATCH(p1:Person{RN:" + rn + "})<-[pp1:path_person]-(f1:fam_path)-[p11:path_intersect] -(i:intersect) with  i.persons as pi return pi";
        List<Object> ppath = gen.neo4jlib.neo4j_qry.qry_obj_list(cq);
        ...
        String mpaths[] = gen.neo4jlib.neo4j_qry.qry_to_csv("match path=(p1:Person{RN:" + rn + "})-[r1:father|mother*0..25]->(mrca:Person)<-[r2:father|mother*0..25]-(p2:Person) with distinct p2,collect(distinct mrca.RN) as anc_rns with p2,anc_rns where toInteger(left(p2.BD,4))>1940 and p2.DD='' with anc_rns, collect(distinct p2.RN) as rns unwind rns as x call { with x,anc_rns match path3=(p3:Person{RN:x})-[r3:father|mother*0..25]->(p4:Person) where p4.RN in anc_rns and p4.RN<>p3.RN with p3,p4,reduce(s='', x in nodes(path3)|s + x.RN + ';') as ipath return p3.RN as irn,p4.RN as arn, ipath } return irn,arn, ipath").split("\n");
...
//   
     ...
     public static void candidate_report(String fn, Long rn)
     {
         ...
         String cq="LOAD CSV WITH HEADERS FROM 'file:///" + fn + "' as line FIELDTERMINATOR '|' with line.proband_anc_rn as proband_anc_rn,line.match_rn as rn,line.match_path as candidate_path,line.intersect as i,collect(distinct line.proband_path) as pp with proband_anc_rn,rn,candidate_path, i,pp return rn as candidate,proband_anc_rn,'[' + candidate_path + ']' as candidate_path, '[' + i + ']' as intersection,'[' + head(pp) + '] ' as proband_path order by  candidate,proband_anc_rn";
                 //"LOAD CSV WITH HEADERS FROM 'file:///" + fn + "' as line FIELDTERMINATOR '|' with line.proband_anc_rn as proband_anc_rn,line.match_rn as rn,line.intersect as i,collect(distinct line.proband_path) as pp with proband_anc_rn,rn, i,pp  return rn as candidate,proband_anc_rn, '[[' + i  + ']]' as match_path,size(pp) as proband_path_ct,'[[' + head(pp) + ']]' as proband_path order by proband_path_ct desc, candidate";
         
         ////full report--too many rows for Excel from GFG
        //String cqq = "LOAD CSV WITH HEADERS FROM 'file:///" + fn + "' as line FIELDTERMINATOR '|' with line.proband_anc_rn as proband_anc_rn,line.match_rn as rn,line.match_path as candidate_path,line.intersect as i,collect(distinct line.proband_path) as pp with proband_anc_rn,rn,candidate_path, i,pp  return rn as candidate,proband_anc_rn,candidate_path, '[' + i + ']]' as match_path,size(pp) as proband_path_ct,'[[' + head(pp) + ']] ' as proband_path order by proband_path_ct desc, candidate";
        
         
         String excelFile = gen.excelLib.queries_to_excel.qry_to_excel(cq, "path_triangulation_candidates for proband_" + rn, "detailed_list", ct, "", "0:######;1:######;3:######", "", false,"Find putatively living relates of the proband, " + proband + " whose family tree path intersect.\n\n Data is rendered into a csv file that resides in the import directory\nCypher queries are run against the data in this file. \n\nCypher query:\n" + cq + "\n\n", false);
         ct = ct +1;
         ...
         cq = "LOAD CSV WITH HEADERS FROM 'file:///" + fn + "' as line FIELDTERMINATOR '|'  with line.match_rn as c,apoc.coll.sort(collect(distinct line.proband_anc_rn)) as arn with c,arn  return gen.gedcom.person_from_rn(toInteger(c), true) as candidate,size(arn) as ancestor_ct,arn as ancestor_rns order by size(arn)";
                 //"LOAD CSV WITH HEADERS FROM 'file:///" + fn + "' as line FIELDTERMINATOR '|'  with line.proband_anc_rn as proband_anc_rn,line.match_rn as rn,line.intersect as i, collect(distinct line.proband_path) as pp  with proband_anc_rn,rn, i,pp where size(pp) = 1  with  distinct proband_anc_rn, rn  with  rn,collect(distinct proband_anc_rn) as rns,count(*) as ct  return gen.gedcom.person_from_rn(toInteger(rn),true) as path_triangulation_candidate, size(rns) as unique_candidates,ct";
         ...
         cq= "LOAD CSV WITH HEADERS FROM 'file:///" + fn + "' as line FIELDTERMINATOR '|'  with line.proband_anc_rn as p,apoc.coll.sort(collect(distinct line.match_rn)) as crn with p,crn  return gen.gedcom.person_from_rn(toInteger(p), true) as proband_ancestor,size(crn) as candidate_ct,crn as candidate_rns order by size(crn)";
                 //"LOAD CSV WITH HEADERS FROM 'file:///" + fn + "' as line FIELDTERMINATOR '|'  with line.proband_anc_rn as proband_anc_rn,line.match_rn as rn,line.intersect as i, collect(distinct line.proband_path) as pp  with proband_anc_rn,rn, i,pp where size(pp) = 1  with  distinct proband_anc_rn, rn  with  proband_anc_rn,collect(distinct rn) as rns,count(*) as ct  return gen.gedcom.person_from_rn(toInteger(proband_anc_rn),true) as proband_ancestor, size(rns) as unique_candidates,ct";
