...
//        ct=ct+1;
        ...
        cq = "match (p:Person) where p.surname>'A' return p.fullname as Name, p.RN as RN, p.BDGed as BD,p.DDGed as DD, case when trim(p.kit)='' then ' ' else p.kit end as Kit order by p.surname, p.first_name, p.middle_name";             
        excelFile = gen.excelLib.queries_to_excel.qry_to_excel(cq, "persons", "persons", ct, "", "1:#####",  excelFile, false, cq, false);
        ct=ct+1;

        cq = "MATCH p=(k:Kit)-[r:KitMatch]->(m:DNA_Match) RETURN k.kit as kit,count(*) as matches order by kit";             
