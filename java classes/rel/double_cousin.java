...
public class double_cousin {
    ...
    public static void main(String args[]) {
     ...
     public static String get_double_cousins()
    {
        ...
        String ef = gen.excelLib.queries_to_excel.qry_to_excel(cq, "double cousins", "double_cousins", 1, "", "1:####", "", false, "UDF\nreturn gen.rel.double_cousin_reports()\n\ncypher query\n"+ cq + "\n\nDouble cousins share two sets of grandparents.", false);
        
        cq = "Match (p:Person), (q:Person) match path=(p:Person)-[[:father|mother*..2]]->(CA)<-[[:father|mother*..2]]-(q:Person) with p, q,CA where p.fullname=replace(p.fullname,'MRCA','') and q.fullname=replace(q.fullname,'MRCA','') with p.fullname + ' ⦋' + p.RN + '⦌ (' + left(p.BD,4) + '-' + left(p.DD,4) + ')' as Name1, q.fullname + ' ⦋' + q.RN + '⦌ (' + left(q.BD,4) + '-' + left(q.DD,4) + ')' as Name2, CA.fullname + ' ⦋' + CA.RN + '⦌ (' + left(CA.BD,4) + '-' + left(CA.DD,4) + ')' as MRCAs with Name1,Name2,MRCAs order by MRCAs with Name1,Name2,collect(MRCAs) as mrcas, count(*) as MRCA_Ct where MRCA_Ct=3 with apoc.coll.dropDuplicateNeighbors(apoc.coll.sort(apoc.coll.flatten(collect(Name1) + collect(Name2)))) as double_cousins, mrcas return distinct mrcas as grandparents,size(double_cousins) as ct,double_cousins order by grandparents";
           gen.excelLib.queries_to_excel.qry_to_excel(cq, "half double cousins", "half double_cousins", 1, "", "1:####", ef, true, "UDF\nreturn gen.rel.double_cousin_reports()\n\ncypher query\n"+ cq + "\n\nDouble cousins share two sets of grandparents.", false);
        
        //update double cousin relationships
        gen.neo4jlib.neo4j_qry.qry_write("match path=(p:Person)-[:father|mother*..2]->(CA)<-[:father|mother*..2]-(q:Person) with p.RN as RN1,p.fullname as Name1,q.RN as RN2,q.fullname as Name2, count(*) as MRCA_Ct where MRCA_Ct>3 and p.fullname=replace(p.fullname,'MRCA','') and q.fullname=replace(q.fullname,'MRCA','') with distinct RN1,RN2 match (m1:DNA_Match{RN:toInteger(RN1)})-[r:match_by_segment]-(m2:DNA_Match{RN:toInteger(RN2)}) set r.rel='DC'");
