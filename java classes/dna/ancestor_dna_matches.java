...
//            Long rn2
    ...
    public static String ancestor_descendant_matches(Long rn) {
     ...
     int ct=1;
        String cq = "match (n:Person)-[[z:Gedcom_DNA]]->(m) with collect(m.RN) + collect(n.RN) as DM optional match path=(p:Person{RN:" + rn + "})<-[[:father|mother*0..15]]-(q:Person) where q.RN in DM with p,path,collect(last(nodes(path))) as cEnds optional match (q:Person)-[[r:Gedcom_DNA]]->(s:Person) where q in cEnds with r,p,[[m in cEnds|m.fullname + gen.neo4jlib.RNwithBrackets(m.RN)]] as E,[[n in nodes(path)|n.fullname +  gen.neo4jlib.RNwithBrackets(n.RN) + ' (' + left(n.BD,4) + '-' +  left(n.DD,4) + ')  ' ]] as N, [[rn in nodes(path)|rn.RN]] as R return distinct p.fullname + gen.neo4jlib.RNwithBrackets(p.RN) as MRCA, E as Descendant_DNA_Tester,size(N) as generations,R as RN_in_ORDPATH_order, N as Path_to_Descendant_Tester order by gen.graph.get_ordpath(R)";
       try{
        String excelFile = gen.excelLib.queries_to_excel.qry_to_excel(cq,"descendant_matches","Descendants", ct, "2:10;3:75", "2:####;2:####", "", false,"UFD: \nreturn gen.dna.ancestor_descendants_with_dna_test(" + rn + ")\n\ncypher query\n" + cq,true);
        ct = ct +1;

try{        cq = "match (n:Person)-[[z:Gedcom_DNA]]->(m) with collect(m.RN) + collect(n.RN) as DM optional match path=(p:Person{RN:" + rn + "})<-[[:father|mother*0..99]]-(q:Person) where q.RN in DM with p,path,collect(last(nodes(path))) as cEnds optional match (q:Person)-[[r:Gedcom_DNA]]->(s:Person) where q in cEnds with r,p,[[m in cEnds|m.fullname]] as E with apoc.coll.sort(apoc.coll.flatten(collect (distinct E))) as desc_tester match (m1:DNA_Match)-[[rs:match_segment]]->(s:Segment) where rs.p in desc_tester and rs.m_anc_rn=" + rn + " with s,collect(distinct m1.fullname) as probands,collect(distinct rs.m) as matches return s.chr as chr,s.strt_pos as strt_pos,s.end_pos as end_pos,gen.tgs.segment_tgs(s.Indx,'" + gen.neo4jlib.neo4j_info.project + "') as tg,size(probands) as ct,probands,size(matches) as ct2,matches order by s.chr,s.strt_pos,s.end_pos";
...
ct = ct+1;
  ...
  try{      cq = "match (n:Person)-[[z:Gedcom_DNA]]->(m) with collect(m.RN) + collect(n.RN) as DM optional match path=(p:Person{RN:" + rn + "})<-[[:father|mother*0..15]]-(q:Person) where q.RN in DM with p,path,collect(last(nodes(path))) as cEnds optional match (q:Person)-[[r:Gedcom_DNA]]->(s:Person) where q in cEnds with r,p,[[m in cEnds|m.fullname]] as E with apoc.coll.dropDuplicateNeighbors(apoc.coll.sort(apoc.coll.flatten(collect (distinct E)))) as desc_tester match (m1:DNA_Match)-[[rs:match_tg]]->(t:tg) where rs.p in desc_tester and rs.m_anc_rn=" + rn + " with t,apoc.coll.dropDuplicateNeighbors(apoc.coll.sort(apoc.coll.flatten(collect(distinct m1.fullname) + collect(distinct rs.m)))) as matches,apoc.coll.dropDuplicateNeighbors(apoc.coll.sort(apoc.coll.flatten(collect(distinct m1.RN) + collect(distinct rs.m_rn)))) as rns return t.tgid as tgid,t.chr as chr,t.strt_pos as strt_pos,t.end_pos as end_pos,t.cm as cm,matches,collect(gen.rel.mrca_from_cypher_list(rns,15)) as mrcas order by t.chr,t.strt_pos,t.end_pos";
        ...
        gen.excelLib.queries_to_excel.qry_to_excel(cq,"descendant_tgs","tgs", ct, "2:10;3:75", "0:###;1:####;2:###,###,###;3:###,###,###;4:###.#", excelFile, true,"UFD: gen.dna.ancestor_descendants_with_dna_test(" + rn + ")\n\ncypher query\n" + cq + "\n\n" , true);
