...
public class ancestor_seg_property {
     ...
     public String phase() 
    {
        ...
        gen.neo4jlib.neo4j_qry.qry_write("LOAD CSV WITH HEADERS FROM 'file:///phased_segs.csv' as line FIELDTERMINATOR '|' match (s:Segment) where id(s)=toInteger(line.id_seg) set s.anc_desc=" + anc_rn);

        //Add property to match_segment
        gen.neo4jlib.neo4j_qry.qry_write("LOAD CSV WITH HEADERS FROM 'file:///phased_segs.csv' as line FIELDTERMINATOR '|' match (m:DNA_Match)-[r:match_segment]->(s:Segment) where id(r)=toInteger(line.id_phased_rel) set r.anc_desc=" + anc_rn);
