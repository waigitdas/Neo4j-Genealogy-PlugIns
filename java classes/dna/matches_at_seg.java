...
public class matches_at_seg {
     ...
     public String get_matches(String chr,Long strt, Long end)
    {
        ...
        gen.excelLib.queries_to_excel.qry_to_excel(cq, "Matches at chr_region", "Region matches", 1, "", "1:###.#;2:####.#;3:####;4:###", "", true, "UDF: gen.dna.matches_at_chr_region(" + chr + ", " + strt + ", " + end + ")\n\nmatches at chr " + chr + " between positions " + strt + " and " + end + " where the * prefix indicates those who are identified.\nThe number in brackets {} are the centimorgans shared with the match (column A)\n\nthe query is\n" + cq,true);
        r = "completed";  // gen.genlib.listStrToStr.list_to_string(gen.neo4jlib.neo4j_qry.qry_str_list(cq));
    }
