...
public class segment_desc_matches {
    ...
    String cq = "match (s:Segment)-[r:match_segment{m_anc_rn:41,p_anc_rn:41,p:m.fullname}]-(m:DNA_Match{ancestor_rn:41}) where s.chr='" + chr + "' and s.end_pos>=" + strt + " and s.strt_pos<= " + end + "with m order by m.fullname with collect (distinct m.fullname + ' [' + m.RN + ']') as match return match";
