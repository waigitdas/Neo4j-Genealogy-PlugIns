...
public class avatar_parental_cm {
     ...
     public static String get_cm() 
    {
       ...
       gen.neo4jlib.neo4j_qry.qry_write("MATCH p=()-[r:avatar_cseg]->() delete r");
       gen.neo4jlib.neo4j_qry.qry_write("MATCH p=()-[r:cseg_seg]->() delete r");
       gen.neo4jlib.neo4j_qry.qry_write("match (a:CSeg) delete a");
       
       //merge segments, computertheir cm and create CSegs
        ...
        //get cm data
        gen.neo4jlib.neo4j_qry.qry_write("MATCH (a:CSeg) with a,gen.dna.hapmap_cm(case when a.chr='0X' then '23' else a.chr end,a.strt_pos,a.end_pos) as cm set a.cm=cm ");
        
        gen.neo4jlib.neo4j_qry.qry_write("MATCH p=(a:Avatar)-[r:avatar_cseg]->(s) where r.side='paternal' with a,sum(s.cm) as cm set a.paternal_cm = cm");
        gen.neo4jlib.neo4j_qry.qry_write("MATCH p=(a)-[r:avatar_cseg]->(s) where r.side='maternal' with a,sum(s.cm) as cm set a.maternal_cm = cm");
        //gen.neo4jlib.neo4j_qry.qry_write("MATCH p=(a)-[r:avatar_cseg]->(s) where r.side='total' with a,sum(s.cm) as cm set a.total_cm = cm");
        ...
        String cq = "MATCH (a:Avatar) where a.segment_coverage is not null RETURN a.fullname as Avatar, a.RN as RN, toInteger(case when a.paternal_cm is null then 0 else a.paternal_cm end) as paternal_cm, toInteger(case when a.maternal_cm is null then 0 else a.maternal_cm end) as maternal_cm,a.total_cm as total_cm order by a.total_cm desc";
         gen.neo4jlib.neo4j_qry.qry_write("MATCH p=(a)-[r:avatar_cseg]->(s:CSeg)  with distinct a,a.fullname as fn,s with a,fn,sum(s.cm) as cm set a.segment_coverage=cm/" + total_expected_cm + ",a.total_cm=cm");
