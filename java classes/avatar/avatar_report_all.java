...
public class avatar_report_all {
     ...
     public static String create_report() 
    {
      ...
      //avatar list by origin
       cq ="MATCH p=(v:Avatar)-[r:avatar_segment]->(s:Segment) with r,v,s optional MATCH p2=(v)-[ra:avatar_cseg]->(av:CSeg) with v.fullname as fullname,v.RN as RN,v.total_cm as total_cm,v.segment_coverage as segment_coverage,case when v.paternal_cm is not null then v.paternal_cm else '.' end as paternal_cm,case when v.maternal_cm is not null then v.maternal_cm else '.' end as maternal_cm, apoc.coll.sort(collect(distinct s.Indx)) as MSegs, size(collect(distinct av.Indx)) as CSeg_ct,apoc.coll.sort(collect(distinct r.source)) as sources return fullname as avatar,RN, size(MSegs) as MSeg_ct, CSeg_ct,total_cm,segment_coverage,paternal_cm,maternal_cm, sources,MSegs order by case when total_cm is null then 0 else total_cm end desc,avatar";
               //"MATCH p=(v:Avatar)-[[r:avatar_segment]]->(s:Segment) with v.fullname as fullname,v.RN as RN, count(*) as seg_ct,apoc.coll.sort(collect(distinct r.source)) as sources return fullname,RN, seg_ct, sources order by seg_ct desc";
       excelFile = gen.excelLib.queries_to_excel.qry_to_excel(cq, "Avatar_list", "avatars", ct, "", "1:######;2:###,###;3:######;4:#####;4:########;5:0.####;6:####;7:#####", "", false, "UDF:\n" + UDF_query + "\n\ncypher query:\n" + cq + "\n\nList of avatar and their associated data. MSegs are consolidated into CSegs when their bundaries overlap. Unlike other shared cM reporting, avatar reports include both maternal and paternal segments. ", false);
       ct = ct+1;
      ...
      //avatar shared matches
       ...
       gen.excelLib.queries_to_excel.qry_to_excel(cq, "Avatar_shared_matches", "avatar_shared_matches", ct, "", "2:###,###;3:###,###.##;4:###,###.##;5:#,###.##;6:###,###.##;7:###,###.##;8:###.##;9:##.##", excelFile, false, "UDF:\n" + UDF_query + "\n\ncypher query:\n" + cq, false);
       ct = ct+1;
      ...
      gen.excelLib.queries_to_excel.qry_to_excel(cq, "Avatar_discovery", "discovered_matches", ct, "", "1:######;2:###,###", excelFile, false, "UDF:\n" + UDF_query + "\n\ncypher query:\n" + cq + "\n\nAvatar segments shared with actual DNA testers who do not have a record number in the curated file and/or family tree.", false);
       ct = ct+1;

//       cq = "MATCH p=(a:Avatar)-[[r:avatar_segment]]->(s) with a,s,r match (d:DNA_Match)-[[rs:match_segment]]->(s) where d.RN is null with distinct a,s,r,d,rs with distinct d.fullname as match, d.RN as match_rn,r.cm as shared_cm,count(*) as seg_ct,rs.p_rn as p_rn,rs.m_rn as m_rn, rs.p as p, rs.m as m, collect(distinct s.Indx) as segs,collect( distinct a.fullname) as avatars,apoc.coll.dropDuplicateNeighbors(apoc.coll.sort(apoc.coll.flatten(collect(r.mrca_rn)))) as mrca_rns where shared_cm>=7 with match,match_rn,size(avatars) as source_avatar_ct,shared_cm,seg_ct,p_rn,m_rn,p,m, segs,avatars,mrca_rns return match,size(avatars) as source_avatar_ct,shared_cm,seg_ct,p_rn,p, segs,avatars,mrca_rns order by match";
//          gen.excelLib.queries_to_excel.qry_to_excel(cq, "new_matches", "new_matches", ct, "", "1:######;2:###,###;3:######;4:#####;4:########;5:0.####;6:####;7:#####", excelFile, false, "UDF:\n" + UDF_query + "\n\ncypher query:\n" + cq + "\n\nAvatar segments shared with actual DNA testers who do not have a record number in the curated file and/or family tree.", false);
//       ct = ct+1;
//   
       
       cq ="match (d:Avatar) return labels(d) as item,count(*) as ct union match ()-[[r:avatar_segment]]->() return type(r) as item,count(*) as ct";
        gen.excelLib.queries_to_excel.qry_to_excel(cq, "Avatar_report", "nodes and rel", ct, "", "1:#,###2:####", excelFile, false,"UDF:\n" + UDF_query + "\n\ncypher query:\n" + cq, false);
       ct = ct+1;
...
//       
       ...
       //sources
       cq ="MATCH (a:Avatar)-[[r:avatar_segment]]->() with a,r match (p:Person{RN:r.source}) with p.fullname + ' ⦋' + p.RN + '⦌' as source,collect(distinct a.fullname + ' ⦋' + a.RN + '⦌') as avatars return source,size(avatars) as avatar_ct, avatars";
        gen.excelLib.queries_to_excel.qry_to_excel(cq, "Avatar_sources", "source_kits", ct, "", "1:######;2:###,###;4:###,###;5:###,###;6:###,###", excelFile, false, "UDF:\n" + UDF_query + "\n\ncypher query:\n" + cq + "\n\nFTDNA kits that were the source of segments attributed to avatars.", false);
       ct = ct+1;
...
//        gen.excelLib.queries_to_excel.qry_to_excel(cq, "Coverage_obs_expected", "segment_coverage", ct, "", "1:0.########;2:0.########;3:0.########", excelFile, false, "UDF:\n" + UDF_query + "\n\ncypher query:\n" + cq + "\n\nSegment coverage aobserved compared to the expected computed from the testers available\n\nGFG uses the term segment coverage to distinguish its meaning from DNA coverage.\nDNA coverage, in GFG, is a theoretical amount of DNA cM expected from a set of testers.\nsegment coverage is the actual cM attributable to avatars.", false);
//       ct = ct+1;
       ...
       gen.excelLib.queries_to_excel.qry_to_excel(cq, "Arvatar_names", "attribution_method", ct, "", "1:######;2:0.####;3:###,###;4:###,###;5:###,###;6:###,###", excelFile, false, "UDF:\n" + UDF_query + "\n\ncypher query:\n" + cq + "\n\nAvatar cM assigned to a parental side by each method. The cM are for MSegs and the totals are thus double counted because the overlap. \nThis report help us understand the relative contribution of the different methods to the cM.", false);
       ct = ct+1;
...
//       
       ...
       gen.excelLib.queries_to_excel.qry_to_excel(cq, "Avatar_segs", "segments", ct, "", "1:######;2:###,###", excelFile, false, "UDF:\nr" + UDF_query + "\n\ncypher query:\n" + cq, false);
       ct = ct+1;
