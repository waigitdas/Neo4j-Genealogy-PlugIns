...
public class anc_desc_segs {
    ...
    public static void main(String args[]) {
     ...
     public String get_data(Long rn) 
    {
        ...
        String excelFile = gen.excelLib.queries_to_excel.qry_to_excel(cq,gen.neo4jlib.neo4j_info.project + "_ancestor_descendant_segs", "Rollup", 1, "", "0:#,###;1:#,###;4:#,###;7:#,###", "", true, "cypher qury\n" + cq + "\n\nThis is a summary of the descendants and their segments.\nColumn A is the total number of descendant; column B the escendants with at-DNA test results; columns C and D a list of them; coumns E,  F, and G re all descendant matches in the GEDCOM.\ncolumns H and I are the segments reported out in the DNA Painter file by match-pair.\n\n The accompanying DNA Painter file has the segments of the descentants of " + gen.gedcom.get_family_tree_data.getPersonFromRN(rn, true) + ".\n\nReferences:\nhttps://dna-explained.com/2014/10/03/ancestor-reconstruction/?fbclid=IwAR254i11oH1ibwGYl0xeMn2S6R5Vk_xkXO14OAARta6WvGRGfmgtrN-rwv4\nhttps://dna-explained.com/category/ancestor-reconstruction/", true);
        
        cq = "match path=(p:Person{RN:" + rn + "})<-[:father|mother*0..15]-(q:Person) with collect (distinct q.RN) as rns match (m:DNA_Match)-[rs:match_segment]->(s:Segment) where m.RN in rns and rs.cm>=7 and rs.snp_ct>=500 and gen.dna.chr_portion_of_segment(s.chr,id(rs))<0.5 with rns,collect(distinct s.Indx) as segs,apoc.coll.sort(collect(distinct m.RN)) as rns_desc_tester,apoc.coll.sort(collect(distinct m.fullname)) as fn_desc_tester match (m2:DNA_Match)-[rs2:match_segment]->(s2:Segment) where s2.Indx in segs with rns,segs,apoc.coll.sort(collect(distinct m2.RN)) as rns_all,apoc.coll.sort(collect(distinct m2.fullname)) as fn_all,rns_desc_tester,fn_desc_tester with rns,segs,rns_all,rns_desc_tester,fn_all,fn_desc_tester match (m3:DNA_Match)-[rs3:match_segment]->(s3:Segment) where s3.Indx in segs and m3.fullname in fn_desc_tester and rs3.p_anc_rn>0 and rs3.m_anc_rn>0 and gen.dna.chr_portion_of_segment(s3.chr,id(rs3))<0.5 with case when rs3.p<rs3.m then rs3.p + ':' + rs3.m else rs3.m + ':' + rs3.p end as fn3,s3,rs3 return distinct toInteger(case when s3.chr='0X' then 23 else s3.chr end) as chr,s3.strt_pos as start, s3.end_pos as end, rs3.cm as cm,rs3.snp_ct as snps, fn3 as match,'good' as confidence,rs3.pair_mrca as group,'maternal' as side, '' as notes,'' as color";
        //gen.excelLib.queries_to_excel.qry_to_excel(cq,gen.neo4jlib.neo4j_info.project + "_ancestor_descendant_segs", "dna_painter", 2, "", "", excelFile, true, cq, true);
        gen.neo4jlib.neo4j_qry.qry_to_csv(cq, gen.neo4jlib.neo4j_info.project + "_DNA_Painter_ancestor_" + rn + "_desc_segments_" + gen.genlib.current_date_time.getDateTime() + ".csv");
        
        return "completed\n\nDNA Paintr file in import directory.";
    }
}
