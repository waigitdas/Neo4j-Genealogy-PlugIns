...
public class matches_shared_tgs {
    ...
    public static void main(String args[]) {
     ...
     public String get_matches(Long min_cm,Long max_cm) 
    {
        if(gen.neo4jlib.neo4j_info.tg_file.equals("")){return "";}
            
        String cq ="MATCH p=(m:DNA_Match)-[r:match_tg]->(t:tg) where m.ancestor_rn is not null and " + max_cm + ">r.min_cm>" + min_cm + " with t,m,r, sum(case when m.RN is not null then 1 else 0 end) as mm_ct , case when m.RN is not null then '*' + trim(m.fullname) + ' ⦋' + m.RN + '⦌' else m.fullname end as mm, case when r.m_rn is not null then '*' + trim(r.m) + ' ⦋' + r.m_rn + '⦌' else r.m end as mth with t,apoc.coll.sort(collect(distinct mm)) as propositi, collect(distinct mth) as mth2,collect(distinct m.RN) as rns with t,propositi,apoc.coll.sort(mth2) as mth3 with t,propositi,apoc.coll.dropDuplicateNeighbors(mth3) as mth4 with t,propositi,apoc.coll.disjunction(mth4,propositi) as matches RETURN t.tgid as tg,t.name as tg_name,t.chr as chr, t.strt_pos as strt_pos,t.end_pos as end_pos,t.cm as cm,size(propositi) as ct,propositi,size(matches) as ct_matches, apoc.coll.sort(matches) as matches order by chr,strt_pos,end_pos"; 
