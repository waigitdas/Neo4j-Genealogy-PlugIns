...
public class matches_from_clustered_matches {
    ...
    {
        String cq = "match (k:Kit) where k.ancestor_rn is not null with collect(k.RN) as krns MATCH (k1:Kit)-[[r1:KitMatch]]->(f:DNA_Match)<-[[r2:KitMatch]]-(k2:Kit) where k1.RN in krns and k2.RN in krns and " + max_cm + ">=r1.sharedCM>=" + min_cm + " and " + max_cm + ">=r2.sharedCM>=" + min_cm + " and k1<>k2 with f,apoc.coll.dropDuplicateNeighbors (apoc.coll.sort(collect(k1.fullname) + collect(k2.fullname))) as ck,apoc.coll.dropDuplicateNeighbors (apoc.coll.sort(collect(k1.RN) + collect(k2.RN))) as crn with case when f.RN is not null then '*' + f.fullname + ' ⦋' + f.RN + '⦌' else f.fullname end as fullname,size(crn) as ct,ck,crn with fullname,ct,ck,crn where ct>" + min_cluster_size + " -1 with fullname,ct,ck,crn order by fullname with collect(fullname) as fn,ct,ck,crn  with ct,ck,crn,fn with ct as Kit_ct,size(fn) as Match_ct,ck as Kits,crn as Kit_RNs,fn as Matches with Kit_ct,Kit_ct+Match_ct as Total,Kits,Kit_RNs,Matches where Total<50 with Kit_ct, Total,Kits,Kit_RNs,Matches,gen.rel.mrca_from_cypher_list(Kit_RNs," + mrca_generations + ") as mrca return Kit_ct, Total,Kits,mrca,Kit_RNs,Matches  order by Total desc";
