...
public class matches_from_clustered_matches {
     ...
     public static String get_matches(Long min_cluster_size,Long min_cm, Long max_cm) 
    {
        ...
        String excelFile = gen.excelLib.queries_to_excel.qry_to_excel(cq, "cluster_matches", "matches", ct, "", "0:###;1:###", "", false, "UDF: \nreturn gen.discover.match_clusters(" + min_cluster_size + ", " + min_cm + ", " + max_cm + ")\n which is return gen.discover.match_clusters(min_cluster_size,min_cm, max_cm)\n\ncypher:\n" + cq + "\n\nThis function finds clusters of kits (known peope sharing the common ancestor) which are in-common-with matches to each other and then \nfinds those matches who are also in-common-with all in the cluster\nThese discovered matches are good candidates for further research.\nNew matches (column F) are likely to be in the line of the MRCAs identified from the known members of the cluster (column C).\nThe odds of this are increased with larger cluter sizes (column A).\n\nThe full query which will run in Neo4j Browser is \n" + cq + "\n\nThere are columns in the report with queries which drill down into the cluster:\nDNA Painter query: Run the query, save the file and then load to DNA Painter\nGraphXR query: Run the query in GraphXR and visualize as a circle. ",true );
        ct = ct + 1;
        
        cq = "match (k:Kit) where k.ancestor_rn is not null with apoc.coll.sort(collect(k.RN)) as krns MATCH (k1:Kit)-[r1:KitMatch]->(f:DNA_Match)<-[r2:KitMatch]-(k2:Kit) where k1.RN in krns and k2.RN in krns and " + max_cm + ">=r1.sharedCM>=" + min_cm + " and " + max_cm + ">=r2.sharedCM>=" + min_cm + " and k1<>k2 with f,apoc.coll.dropDuplicateNeighbors (apoc.coll.sort(collect(k1.fullname) + collect(k2.fullname))) as ck,apoc.coll.dropDuplicateNeighbors (apoc.coll.sort(collect(k1.RN) + collect(k2.RN))) as crn with f,size(crn) as ct,ck,crn with case when f.RN is not null then '*' + f.fullname + ' ⦋' + f.RN + '⦌' else f.fullname end as fullname,ct,ck,crn where ct>" + min_cluster_size + "  -1 with fullname,ct,ck,crn order by fullname with collect(fullname) as fn,ct,ck,crn  with ct,ck,crn,fn with ct as Kit_ct,size(fn) as Match_ct,ck as Kits,crn as Kit_RNs,fn as Matches with Kit_ct,Kit_ct+Match_ct as Total,Kits,Kit_RNs,Matches where Total<50 with Kit_ct, Total,Kits,Kit_RNs,gen.rel.mrca_from_cypher_list(Kit_RNs,15) as mrcas, Matches with mrcas, apoc.coll.dropDuplicateNeighbors( apoc.coll.sort(apoc.coll.flatten(collect(Matches)))) as matches return mrcas, size(matches) as ct, matches";
        gen.excelLib.queries_to_excel.qry_to_excel(cq, "cluster_matches", "matches by mrcas", ct, "", "0:###;1:###", excelFile, true, "UDF: \nreturn gen.discover.match_clusters(" + min_cluster_size + ", " + min_cm + ", " + max_cm + ")\n which is return gen.discover.match_clusters(min_cluster_size,min_cm, max_cm)\n\ncypher:\n" + cq + "\n\nThis function finds clusters of kits (known peope sharing the common ancestor) which are in-common-with matches to each other and then \nfinds those matches who are also in-common-with all in the cluster\nThese discovered matches are good candidates for further research.\nNew matches (column F) are likely to be in the line of the MRCAs identified from the known members of the cluster (column C).\nThe odds of this are increased with larger cluter sizes (column A).\n\nThe full query which will run in Neo4j Browser is \n" + cq + "\n\nThere are columns in the report with queries which drill down into the cluster:\nDNA Painter query: Run the query, save the file and then load to DNA Painter\nGraphXR query: Run the query in GraphXR and visualize as a circle. ",true );
        ct = ct + 1;
