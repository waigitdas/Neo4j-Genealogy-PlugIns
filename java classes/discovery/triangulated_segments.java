     ...
     public String get_matches_segments(Long match_type, Long min_cm, Long max_cm,Boolean known_matches_only)
    {
       ...
       Double vcor =0.125;
        ...
        if (known_matches_only ) {
            ...
            cqv = "CALL gds.graph.project.cypher('shared_matches','MATCH (m:DNA_Match) where m.RN is not null RETURN id(m) AS id', 'MATCH (m)-[r:shared_match]->(m2:DNA_Match) where r.cor<=" + vcor + " and " + max_cm + ">r.cm>" + min_cm + " RETURN id(m) AS source, id(m2) AS target, r.cm as weight',{readConcurrency: 4,validateRelationships:FALSE} ) YIELD graphName AS graph, nodeQuery, nodeCount AS nodes, relationshipQuery, relationshipCount AS rels return nodes, rels";
            }
        }
            else {
