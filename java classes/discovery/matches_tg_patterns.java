...
public class matches_tg_patterns {
    ...
    {
        String cq = "match (m:DNA_Match)-[[r:match_tg]]-(t:tg) with m,t order by t.tgid with m,collect(t.tgid) as tc with m, tc where size(tc)>" + min_tg_cluster_size + "-1 return case when m.RN is null then m.fullname else m.fullname + ' ⦋' + m.RN + '⦌' end as fullname,size(tc) as ct,tc order by ct desc";
