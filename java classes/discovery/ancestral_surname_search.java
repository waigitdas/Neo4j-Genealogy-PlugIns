...
public class ancestral_surname_search {
 ...
 try{      
    String excelFile = gen.excelLib.queries_to_excel.qry_to_excel(cq,gen.neo4jlib.neo4j_info.project + "_" + label + "_ancestor_surnames", "matches_with_surname", ct, "","1:###;3:##.##;6:###.#;7:####", "", false,"UDF:\nreturn gen.discovery.ancestral_surnames('" + search_term + "')\n\nCypher query:\n\n" + cq + "\n\nYou can use wildcards to find variations in the surname.\nSee: https://graphaware.com/neo4j/2019/01/11/neo4j-full-text-search-deep-dive.html ", true);
        ...
        ct = ct + 1;
    cq = "CALL db.index.fulltext.queryNodes('ancestor_surnames_names', '" + search_term + "') YIELD node, score WITH ['CUNNINGHAM'] as submitted,score,node.p as match, node.m as match_with_surnames, case when size(node.name)>20000 then left(node.name,200) + ' (truncated)' else node.name end as anc_names , apoc.coll.flatten(collect(split(toUpper(replace(node.name,' ','')),'/'))) as anc_list MATCH (m:DNA_Match{fullname:match})-[rs:match_by_segment]-(m2:DNA_Match{fullname:match_with_surnames}) with distinct m.fullname as source,case when apoc.coll.contains(submitted,toUpper(trim(m2.surname))) then 'x' else '~' end as own_surname, case when m.RN is null then '~' else m.RN end as source_rn, apoc.coll.dropDuplicateNeighbors(apoc.coll.sort(apoc.coll.intersection(anc_list,submitted))) as found, match_with_surnames,rs.cm as cm,rs.seg_ct as segs, case when rs.rel is null then '~' else rs.rel end as rel,round(score,2) as score,anc_names as ancestor_list,submitted,anc_list where found<>[] return match_with_surnames,apoc.coll.sort(collect (distinct source + ' [' + source_rn + ']')),apoc.coll.dropDuplicateNeighbors(apoc.coll.sort(apoc.coll.flatten(collect(distinct found)))) as surnames order by match_with_surnames";
