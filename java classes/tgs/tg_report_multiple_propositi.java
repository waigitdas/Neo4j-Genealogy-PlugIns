...
public class tg_report_multiple_propositi {
    ...
    public String tg_report(
        ...
        String s = get_tg_report(tgid,common_ancestor);
        return s;
...
//    }
//    
      public static String get_tg_report(Long tgid,Long common_ancestor) {
       ...
       String cq = "match p=(m:DNA_Match)-[[rm:match_segment]]-(s:Segment)-[[rs:seg_seq{tgid:" + tgid + "}]]-() where m.ancestor_rn is not null and rm.p_anc_rn is not null and rm.m_anc_rn is not null and 200>=rm.cm>=7 and rm.snp_ct>=500 with s,m.fullname + ' ⦋' + m.RN + '⦌' as propositus, rm.m + ' ⦋' + rm.m_rn + '⦌' as match, rm.cm as cm, rm.snp_ct as snp ,m,rm with m,rm,s,cm, snp, case when m.RN<rm.m_rn then propositus + ', ' + match else match + ', ' + propositus end as propositus_match, case when m.RN<rm.m_rn then 'm' else 'p' end as source with s,propositus_match,source,cm,snp, rm.rel as relationship,gen.rel.mrca_phased(m.RN,rm.m_rn) as mrca order by source desc with propositus_match,relationship,mrca as phased_mrca,s.chr as chr,s.strt_pos as start,s.end_pos as end, cm, snp,collect(distinct source) as pm return propositus_match, relationship,phased_mrca,chr,start,end,cm, snp, pm as source order by propositus_match,start,end";    
