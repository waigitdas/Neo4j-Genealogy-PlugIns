...
//    
      public static String get_tg_report(Long tgid, Long min_cm,Long max_cm, Long common_ancestor) {
       ...
       String cq ="match p=(m:DNA_Match)-[[rm:match_segment]]-(s:Segment)-[[rs:seg_seq{tgid:" + tgid + "}]]-() where m.ancestor_rn is not null and rm.p_anc_rn is not null and rm.m_anc_rn is not null and " + max_cm + " >=rm.cm>=" + min_cm + "  and rm.snp_ct>=500 with s,m.fullname + ' ⦋' + m.RN + '⦌' as propositus, rm.p + ' ⦋' + rm.p_rn + '⦌' as match, rm.cm as cm, rm.snp_ct as snp ,m,rm with m,rm,s,cm, snp, case when m.RN<rm.m_rn then propositus + ', ' + match else match + ', ' + propositus end as propositus_match, case when m.RN<rm.m_rn then 'm' else 'p' end as source with s,propositus_match,source,cm,snp, rm.rel as relationship,gen.rel.mrca_phased(m.RN,rm.p_rn) as mrca order by source desc with propositus_match,relationship,mrca as phased_mrca,s.chr as chr,s.strt_pos as start,s.end_pos as end, cm, snp,collect(distinct source) as pm return propositus_match, case when relationship is null then '--' else relationship end as relationship,phased_mrca,chr,start,end,cm, snp, pm as source order by propositus_match,start,end" ;
