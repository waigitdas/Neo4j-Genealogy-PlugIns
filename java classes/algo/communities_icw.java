...
//            Boolean intermediateCommunities
     ...
     public static String run_algo(Long algorithm, Long min_cm, Long max_cm,Boolean known_matches_only)  //, Boolean intermediateCommunities) 
    {
        ...
        if (algorithm.equals(1L)) {  //louvain
            algo = "louvain";
            algo_nbr = 1L;
            cq = "CALL gds.louvain.stream('icw', {relationshipWeightProperty:'weight',includeIntermediateCommunities:true, tolerance:0.0000001,maxIterations:10,maxLevels:10 }) YIELD nodeId, communityId, intermediateCommunityIds with case when gds.util.asNode(nodeId).RN is not null then '*' + gds.util.asNode(nodeId).fullname else gds.util.asNode(nodeId).fullname end AS name, communityId as cid, intermediateCommunityIds as ici with cid,ici,name order by name with cid,ici,collect(name) as names with cid as community,ici,size(names) as ct, names as matches order by ct desc with community,ici,ct,matches where ct>2 return community,ici as intermediary_communities,ct, matches";
