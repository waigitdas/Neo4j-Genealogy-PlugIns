...
//            Boolean intermediateCommunities
     ...
     public static String run_algo(Long algorithm, Long min_cm, Long max_cm,Boolean known_matches_only)  //, Boolean intermediateCommunities) 
    {
        ...
        if (algorithm.equals(1L)) {  //louvain
            algo = "louvain";
            algo_nbr = 1L;
            cq ="CALL gds.louvain.stream('icw', {relationshipWeightProperty:'weight',includeIntermediateCommunities:true, tolerance:0.0000001,maxIterations:10,maxLevels:10 }) YIELD nodeId, communityId, intermediateCommunityIds with nodeId,case when gds.util.asNode(nodeId).RN is not null then '*' + gds.util.asNode(nodeId).fullname else gds.util.asNode(nodeId).fullname end AS name, communityId as cid, intermediateCommunityIds as ici with nodeId,cid,ici,name order by name with cid,ici,collect(name) as names,collect(gds.util.asNode(nodeId).RN) as rns with rns,cid as community,ici,size(names) as ct, names as matches order by ct desc with rns,community,ici,ct,matches where ct>1 with community,ici as intermediary_communities,ct, matches,gen.rel.mrca_from_cypher_list(rns,15) as mrcas, split(gen.mss.mss_data(rns),'|') as rns return community, intermediary_communities,ct, matches, mrcas,rns[1] as community_seg_ct, rns[0] as mss_phased_anc,rns[2] as mss_in_comm, rns[8] as mss_seg_in_comm, rns[3] as included_mss_total_seg_ct, rns[9] as mss_seg_in_community_segs //,rns[5] as mss_rns , rns[6] as mss_fullnames, rns[7] as community_segs, rns[8] as mss_seg_in_comm2, rns[4] as community_mss_seg_ct";
