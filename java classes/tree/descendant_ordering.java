...
public class descendant_ordering {
    @UserFunction
    @Description("Descendancy tree with generation position.")

    public String add_descendant_order(
        @Name("query") 
            String query,
        @Name("gen_col") 
            Long gen_col
     ...
     public String get_orders(String cq, Long gen_col) 
    {
        ...
        String fn = gen.neo4jlib.neo4j_info.Import_Dir + "ordered_Descendant_tree.csv";
        gen.neo4jlib.file_lib.writeFile(s, fn);
        //gen.excelLib.excel_from_csv.load_csv(fn,gen.neo4jlib.neo4j_info.project +  "_ordered_descendant_tree", "tree", 1, "", "", "", true, "UDF: ", true);
        return s;
    }
}
