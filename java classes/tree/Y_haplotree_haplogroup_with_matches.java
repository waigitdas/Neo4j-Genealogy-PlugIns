...
public class Y_haplotree_haplogroup_with_matches {
     ...
     public String get_tree(String haplogroup)
    {
        ...
        cq = "MATCH path=(b1:block{name:'" + haplogroup + "',ftdna:1})-[[r:blockchild*0..999]]->(b2:block{,ftdna:1}) with b2.name as hg,path,[[m in nodes(path)|id(m)]] as R optional match (m:DNA_Match) where m.YHG=hg with hg,apoc.coll.sort(collect(case when m.ancestor_rn>0 then '*^' + m.fullname else m.fullname end + case when m.RN is not null then ' ⦋' + m.RN + '⦌' else '' end)) as match, R, gen.graph.get_ordpath(R) as op with R,match,hg order by op MATCH p=(b3:block{name:hg,ftdna:1})-[[r:block_snp]]->(v:mt_variant{,ftdna:1}) with R,match,hg,apoc.coll.sort(collect(v.name)) as variants return apoc.text.lpad('',(size(R)-1)*3,'.') + hg as haplogroup ,size(variants) as var_ct,variants,size(R) as haplotree_level, size(match) as match_ct,case when size(match)>100 then 'truncated ' + apoc.coll.remove(match,100,size(match)-99) else match end as match";
